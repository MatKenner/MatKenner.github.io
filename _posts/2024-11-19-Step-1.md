...
This extensive list of services is a tall order for a single device, so we knew that we would need to find a way to have each service run as efficiently as possible. We also wanted to avoid the hefty license fees that enterprise vendors charge. This led us to decide to build the appliance using only open-source, completely free applications.
---
